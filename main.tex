...
\begin{axiom}[The Field Axioms]\label{axiom:1.1}
    \(\left(\symbb{R},+,\cdot\right)\) is a \emph{field}\index[1]{field}, that is to say, let \(\symbb{R}\) be a set,
    \begin{gather*}
    ...
    \end{gather*}
    be two mappings, called \emph{addition}\index[1]{addition} and \emph{multiplication}\index[1]{multiplication}, respectively, satisfying the followings.
    \begin{enumerate}
        \item\label{item:1} For every \(a,b,c\in\symbb{R}\), \(\left(a+b\right)+c=a+\left(b+c\right)\), i.e. addition is \emph{associative}\index[1]{associative law!of addition}.
        \item\label{item:2} There exists a \(0\in\symbb{R}\), s.t. for every \(a\in\symbb{R}\), \(a+0=0+a=a\), and call 0 the \emph{additive identity}\index[1]{identity!of addition}.
        \item\label{item:3} For every \(a\in\symbb{R}\), there exists a \(-a\in\symbb{R}\), s.t. \(a+\left(-a\right)=\left(-a\right)+a=0\), and call \(-a\) the \emph{additive inverse}\index[1]{inverse!of addition} or \emph{opposite}\index[1]{opposite|seealso{inverse, of addition}} of \(a\).
        \item\label{item:4} For every \(a,b\in\symbb{R}\), \(a+b=b+a\), i.e. addition is \emph{commutative}\index[1]{commutative law!of addition}.
        \item\label{item:5} For every \(a,b,c\in\symbb{R}-\set{0}\), \(\left(a\cdot b\right)\cdot c=a\cdot\left(b\cdot c\right)\), i.e. multiplication is \emph{associative}\index[1]{associative law!of multiplication}.
        \item\label{item:6} There exists a \(1\in\symbb{R}-\set{0}\), s.t. for every \(a\in\symbb{R}-\set{0}\), \(a\cdot1=1\cdot a=a\), and call 1 the \emph{multiplicative identity}\index[1]{identity!of multiplication}.
        \item For every \(a\in\symbb{R}-\set{0}\), there exists an \(a^{-1}\in\symbb{R}-\set{0}\), s.t. \(a\cdot a^{-1}=a^{-1}\cdot a=1\), and call \(a^{-1}\) the \emph{multiplicative inverse}\index[1]{inverse!of multiplication} or \emph{reciprocal}\index[1]{reciprocal|seealso{inverse, of multiplication}} of \(a\).
        \item\label{item:8} For every \(a,b\in\symbb{R}-\set{0}\), \(a\cdot b=b\cdot a\), i.e. multiplication is \emph{commutative}\index[1]{commutative law!of multiplication}.
        \item\label{item:9} For every \(a,b,c\in\symbb{R}\), \(a\cdot\left(b+c\right)=\left(a\cdot b\right)+\left(a\cdot c\right)\), \(\left(b+c\right)\cdot a=\left(b\cdot a\right)+\left(c\cdot a\right)\), i.e. multiplication is \emph{distributive}\index[1]{distributive law!of multiplication with respect to addition} with respect to addition.
    \end{enumerate}
\end{axiom}

\begin{remark}
    \Cref{item:1,item:2,item:3} establish that \(\left(\symbb{R},+\right)\) is a \emph{group}\index[1]{group}. The satisfaction of \cref{item:4} further implies that \(\left(\symbb{R},+\right)\) is an \emph{Abelian group}\index[1]{group!Abelian} or \emph{commutative group}\index[1]{group!commutative|seealso{group, Abelian}}. Analogously, \(\left(\symbb{R}^\times\coloneq\symbb{R}-\set{0},\cdot\right)\) is also an Abelian group.

    Taking into account the validity of \cref{item:1,item:2,item:3,item:5,item:9}, we designate the structure \(\left(\symbb{R},+,\cdot\right)\) as a \emph{ring}\index[1]{ring}. If \cref{item:4,item:8} also hold true, it is asserted that \(\left(\symbb{R},+,\cdot\right)\) constitutes a \emph{commutative ring}\index[1]{ring!commutative}.
\end{remark}
...
\begin{axiom}[The Order Axioms]
    Further more, \(\left(\symbb{R},+,\cdot,\leqslant\right)\) is an ordered field, which means \(\left(\symbb{R},+,\cdot\right)\) is a field with total order \(\leqslant\) satisfying the followings.
    \begin{enumerate}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item Since \(-a+a=0\), applying \cref{item:23} with \(-a\) and 0 in place of \(a\) and \(b\) respectively gives the conclusion.\qedhere
    \end{enumerate}
\end{proof}
...
\begin{proof}
    \leavevmode
    \begin{enumerate}
        \item By the distributive law and the cancellation law, \[a\cdot1+a\cdot0=a\cdot\left(1+0\right)=a\cdot1=a\cdot1+0\] implies \(a\cdot0=0\), similarly \(0\cdot a=0=a\cdot0\). In particular, \cref{item:6} in \cref{axiom:1.1} also holds true in \(\symbb{R}\).
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item A few minor modifications to the relevant parts of the above proof are sufficient.
        \item Since \(a^{-1}\cdot a=1\), applying \cref{item:33} with \(a^{-1}\) and 1 in place of \(a\) and \(b\) respectively gives the conclusion.\qedhere
    \end{enumerate}
\end{proof}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item The first equality comes from \[a\cdot b+\left(-a\right)\cdot b=\left(-a+a\right)\cdot b=0\cdot b=0,\] combined with \cref{proposition:1.2}. The other half is proved in the same way.
...
\addtocategory{1}{ded65,rud76,zor24a,zor24b}
...
\emph{Introduction}\index[2]{Introduction}.

\end{section}

\addtocategory{2}{rud76}
