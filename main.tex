...
\begin{remark}
    \Cref{item:1,item:2,item:3} establish that \(\left(\symbb{R},+\right)\) is a \emph{group}\index{group}. The satisfaction of \cref{item:4} further implies that \(\left(\symbb{R},+\right)\) is an \emph{Abelian group}\index{group!Abelian} or \emph{commutative group}\index{group!commutative|seealso{group, Abelian}}. Analogously, \(\left(\symbb{R}^\times\coloneq\symbb{R}-\set{0},\cdot\right)\) is also an Abelian group.

    Taking into account the validity of \cref{item:1,item:2,item:3,item:5,item:9}, we designate the structure \(\left(\symbb{R},+,\cdot\right)\) as a \emph{ring}\index{ring}. If \cref{item:4,item:8} also hold true, it is asserted that \(\left(\symbb{R},+,\cdot\right)\) constitutes a \emph{commutative ring}\index{ring!commutative}.
\end{remark}
