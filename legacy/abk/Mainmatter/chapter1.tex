\begin{chapter}{The Real Numbers and The Integers}
...
\begin{axiom}[The Field Axioms]\label{axiom:1.1}
    \(\left(\symbb{R},+,\cdot\right)\) is a field\index{field}, that is to say, let \(\symbb{R}\) be a set, \[+\colon\symbb{R}^2\to\symbb{R},\quad\left(x,y\right)\mapsto x+y\qquad\text{and}\qquad\cdot\colon\symbb{R}^2\to\symbb{R},\quad\left(x,y\right)\mapsto x\cdot y\] be two mappings, called addition and multiplication, respectively, satisfying the followings.
    \begin{enumerate}
        \item\label{item:1} For every \(a,b,c\in\symbb{R}\), \(\left(a+b\right)+c=a+\left(b+c\right)\), i.e. addition is associative.
        \item\label{item:2} There exists a \(0\in\symbb{R}\), s.t. for every \(a\in\symbb{R}\), \(a+0=0+a=a\), and call 0 the additive identity.
        \item\label{item:3} For every \(a\in\symbb{R}\), there exists a \(-a\in\symbb{R}\), s.t. \(a+\left(-a\right)=\left(-a\right)+a=0\), and call \(-a\) the additive inverse or opposite of \(a\).
        \item\label{item:4} For every \(a,b\in\symbb{R}\), \(a+b=b+a\), i.e. addition is commutative.
        \item\label{item:5} For every \(a,b,c\in\symbb{R}-\set{0}\), \(\left(a\cdot b\right)\cdot c=a\cdot\left(b\cdot c\right)\), i.e. multiplication is associative.
        \item\label{item:6} There exists a \(1\in\symbb{R}-\set{0}\), s.t. for every \(a\in\symbb{R}-\set{0}\), \(a\cdot1=1\cdot a=a\), and call 1 the multiplicative identity.
        \item For every \(a\in\symbb{R}-\set{0}\), there exists a \(a^{-1}\in\symbb{R}-\set{0}\), s.t. \(a\cdot a^{-1}=a^{-1}\cdot a=1\), and call \(a^{-1}\) the multiplicative inverse or reciprocal of \(a\).
        \item\label{item:8} For every \(a,b\in\symbb{R}-\set{0}\), \(a\cdot b=b\cdot a\), i.e. multiplication is commutative.
        \item\label{item:9} For every \(a,b,c\in\symbb{R}\), \(a\cdot\left(b+c\right)=\left(a\cdot b\right)+\left(a\cdot c\right)\), \(\left(b+c\right)\cdot a=\left(b\cdot a\right)+\left(c\cdot a\right)\), i.e. multiplication is distributive with respect to addition.
    \end{enumerate}
\end{axiom}

\begin{remark} 
    \autoref{item:1}, \autoref{item:2}, and \autoref{item:3} establish that \(\left(\symbb{R},+\right)\) is a group. The satisfaction of \autoref{item:4} further implies that \(\left(\symbb{R},+\right)\) is an abelian group or commutative group. Analogously, \(\left(\symbb{R}^\times\coloneq\symbb{R}-\set{0},\cdot\right)\) is also an abelian group.

    Taking into account the validity of \autoref{item:1}, \autoref{item:2}, \autoref{item:3}, \autoref{item:5}, \autoref{item:6}, and \autoref{item:9}, we designate the structure \(\left(\symbb{R},+,\cdot\right)\) as a ring. If \autoref{item:4} and \autoref{item:8} also hold true, it is asserted that \(\left(\symbb{R},+,\cdot\right)\) constitutes a commutative ring.
\end{remark}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item If \(b\) and \(c\) are both additive inverse of \(a\in\symbb{R}\), then by definition, \[b=b+0=b+\left(a+c\right)=\left(b+a\right)+c=0+c=c.\]
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \begin{align*}
            ...
            &\iff\left(-a+a\right)+x=-a+b\iff0+x=-a+b\\
            &\iff x=-a+b.
        \end{align*}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \begin{align*}
            ...
            &\iff\left(a^{-1}\cdot a\right)\cdot x=a^{-1}\cdot b\iff1\cdot x=a^{-1}\cdot b\\
            &\iff x=a^{-1}\cdot b.
        \end{align*}
